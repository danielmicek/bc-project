...
export default function SlidingCircle() {
    ...
    if (screenWidth < 768) {
        ...
        duration-800 ease-in-out"  style={{ top: isOpen ? "5vh" : "75vh" }}>
                ...
                <p id="NOT_SIGNED_YET" className="flex items-center justify-center relative left-5 text-[2rem] font-medium col-start-2 mb-20 row-start-1">Not signed yet?</p>

                <div id="DO_IT_NOW" className="flex flex-col items-center justify-end text-[2rem] font-[1000] col-start-2 row-start-2">
                    <span>Do it now!</span>
                    <span id="GET_ACCESS" className="flex items-center justify-center font-light text-base text-gray-500">And get access to the whole course</span>
                </div>

                <SignedOut>
                    <SignInButton mode="modal" asChild>
                        <Button className="w-[150px] justify-self-center self-start mt-5 font-bold bg-(--main-color-orange) col-start-2 row-start-3">Sign in</Button>
                    </SignInButton>
                </SignedOut>
            </div>
        );
    ...
    return (
        ...
        duration-800 ease-in-out">
            <p id="NOT_SIGNED_YET" className="flex items-center justify-center relative left-[20px] text-[2rem] font-medium col-start-2 row-start-3">Not signed yet?
            </p>
            ...
            <SignedOut>
                <SignInButton mode="modal" asChild>
                    <Button className="w-[150px] justify-self-center self-center font-bold bg-(--main-color-orange) col-start-1 row-start-3">Sign in</Button>
                </SignInButton>
            </SignedOut>
        </div>
    );
